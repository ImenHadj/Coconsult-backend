...
public class CloudinaryController {
    ...
    @PostMapping("/uploadJustifConge/{id}")
    @ResponseBody
    public ResponseEntity<String> uploadJustifConge(@RequestParam MultipartFile multipartFile,@PathVariable("id") Long id) throws IOException {
        BufferedImage bi = ImageIO.read(multipartFile.getInputStream());
        if (bi == null) {
            return new ResponseEntity<>("Image non valide!", HttpStatus.BAD_REQUEST);
        }
        ...
        Image image = new Image((String) result.get("original_filename"),
                (String) result.get("url"),
                (String) result.get("public_id"));
        imageService.saveJustificationConge(image,id);
        return new ResponseEntity<>("image ajoutée avec succès ! ", HttpStatus.OK);
    }
